...
const addSignatureBlock = (worksheet, currentRow) => {
  ...
  cellManager.font = { name: "Arial", size: 14, bold: true, underline: true };
  cellManager.alignment = { horizontal: "center", vertical: "middle" };

  const cellHR = sigRow.getCell(6);
  cellHR.value = "الموارد البشرية";
  cellHR.font = { name: "Arial", size: 14, bold: true, underline: true };
  ...
  lineCellManager.font = { name: "Arial", size: 14, bold: true };
  lineCellManager.alignment = { horizontal: "center", vertical: "bottom" };

  const lineCellHR = lineRow.getCell(6);
  lineCellHR.value = ".........................";
  lineCellHR.font = { name: "Arial", size: 14, bold: true };
...
export const exportToExcel = async (data, dateStr) => {
  try {
    ...
    const { parseISO: pISO } = await import("date-fns");
    ...
    dateCell.value = `   كشف حضور وانصراف (عقود) - ${dateStr}    `;
    dateCell.font = {
      name: "Arial",
      size: 18,
      bold: true,
...
export const exportAllHistory = async (history) => {
  try {
    ...
    for (const date of sortedDates) {
      ...
      const dateCell = worksheet.getCell("B3");
      dateCell.value = `   كشف حضور وانصراف (عقود) - ${date}    `;
      dateCell.font = {
        name: "Arial",
        size: 18,
        bold: true,
    ...
    for (const emp of employees) {
      ...
      daysInMonth.forEach((day, idx) => {
        ...
        const sc = (col, val, f = null, b = false) => {
          ...
          c.border = BORDER_FULL;
          c.alignment = { horizontal: "center", vertical: "middle" };
          c.font = { name: "Arial", size: 12, bold: b };
          if (f) c.numFmt = f;
          if (idx % 2 !== 0)
            c.fill = {
              type: "pattern",
      ...
      const dSi = (col, txt) => {
        ...
        c.value = txt;
        c.font = { bold: true, size: 14, underline: true };
        c.alignment = { horizontal: "center", vertical: "middle" };
      };
